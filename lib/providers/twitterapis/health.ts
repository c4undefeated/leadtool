import { getAccount, TwitterApisNotConfiguredError, TwitterApisRequestError } from "./client";
...
// Thresholds sized around the ~$0.50 initial free credit balance new
// TwitterAPIs accounts start with — a handful of $0.0008 calls' worth of
// headroom, not a real production margin policy.
...
export async function getProviderHealth(force = false): Promise<ProviderHealth> {
  ...
  try {
    ...
    let message = `TwitterAPIs balance: $${remaining.toFixed(2)}.`;
    if (remaining < CRITICAL_BELOW_USD) {
      status = "critical";
      message = `TwitterAPIs balance is critically low ($${remaining.toFixed(2)}) — ingestion is paused to avoid an unexpected failed charge.`;
    } else if (remaining < WARNING_BELOW_USD) {
      status = "warning";
      message = `TwitterAPIs balance is low ($${remaining.toFixed(2)}) — consider topping up soon.`;
    }
    health = { status, message, creditsRemainingUsd: remaining, checkedAt: new Date() };
  } catch (err) {
    if (err instanceof TwitterApisNotConfiguredError) {
      health = {
        status: "not_configured",
        message: "TWITTER_API_KEY not set — X/Twitter ingestion is disabled. Use manual import instead.",
        creditsRemainingUsd: null,
        checkedAt: new Date(),
      };
    } else {
      const message = err instanceof TwitterApisRequestError ? err.message : "TwitterAPIs is unreachable.";
